...
router.post('/api/subscribe', function(req, res) {
	...
	if (errors) {
		...
		if(req.body.productid === req.body.productmeta){
			let subscribed = new Subscribed({...req.body, comboid: req.body.productid, combo: true})
			subscribed.save().then((product) => res.status(200).json({ status: true, product: product }));
		} else {
		const subscribed = new Subscribed(req.body);
		subscribed
			.save()
			.then((product) => res.status(200).json({ status: true, product: product }));
	}}
});
...
router.post('/subscribed/byuser', async function(req, res) {
	...
	let fetchCombos = await Subscribed.find({userid: req.body.userid, combo: true}).populate("comboid")
	//let combos = await fetchCombos.map((el) => {let returnObj; return returnObj = {...el._doc, comboid: el.productid, combo: true}; return returnObj})
	 //console.log(combos);
	let fetchProducts = await Subscribed.find({userid: req.body.userid, combo : false}).populate("productid").populate("productmeta");
