			...
			data.map((order) => {
				if (order.trackerId && order.status !== "delivered") {
					...
					function updateThem(status) {
						...
						OOrder.findByIdAndUpdate(order._id, { $set: { status: currentStatus } }).then((data) => sendMail(data)).catch((err) => res.json({ status: false, err }))
						 ...
						 <tr> 
						  <td class="m_-4653644642374936975emailContainerTD" style="padding-left:23px;padding-right:23px;padding-top:25px;padding-bottom:33px;border-collapse:collapse!important;font-family:Arial,sans-serif"> 
						   <table class="m_-4653644642374936975greetingTable" style="font-size:20px;line-height:24px;color:rgb(0,46,54);margin-bottom:18px;border-collapse:collapse!important;text-align:left"> 
							<tbody>
							 <tr> 
							  <td style="border-collapse:collapse!important;font-family:Arial,sans-serif"><span>Hi ${data.userDetails.firstname},</span> </td> 
							 </tr> 
							</tbody>
						   </table> 
				...
				} else { console.log("no orders to update") }
			})
		}
	}
});
		...
		if (data.length > 0) {
			data.map((order) => {
				if (order.trackerId && order.status !== "delivered") {
					...
					function updateThem(status) {
						let currentStatus = status.status
						OOrder.findByIdAndUpdate(order._id, { $set: { status: currentStatus } }).then((data) => console.log({ status: true, data })).catch((err) => res.json({ status: false, err }))
					}
				} else { console.log("no orders to update") }
			})
		}
...
router.get("/get", (req, res) => {
	...
	OOrder.findById(id).then((data) => res.json({ status: true, data })).catch((error) => res.json({ status: false, error }));
});
...
router.get('/api/getbyuser/:id', function (req, res) {
	...
	OOrder.find({ userMetaId: query }).populate('products')
...
router.get('/review/:id', (req, res) => {
	let id = req.params.id;
	OProduct.findByIdAndUpdate(id, { $set: { reviewed: true } }, { new: true }).then((data) => res.json({ status: true, data })).catch((error) => res.status(400).json({ status: false, error }));
});
...
router.post('/process/order', async function (req, res) {
	...
	if (req.body.products) {
		...
		req.body.products.map(async el => {
			...
			let invItem = {}
			if (el.isSubscribed) { invItem.description = el.title + " " + "(Subscribed Product)" } else {
			...
			function addToUser(data) {
				...
				function processo(data) {
					if (req.body.wasReferred) {
						...
						Referral.findByIdAndUpdate(id, { $set: { converted: true, amount: refAmount, orderid: data._id } }).then(console.log("Referral saved")).catch(console.log);
					}


					let trackerid;
					if(req.body.carrier !== 'shipment_failed'){
					 trackerid = req.body.trackerid;} else {
					 trackerid = "Not found"
					}
					if(req.body.carrier !== 'shipment_failed'){
					api.Tracker.retrieve(trackerid).then(s => sendTrackingId(s)).catch(console.log);} else {
						let dummy = {
				  ...
				  <div class="invoice">
					<table id="m_-1220850270213470107m_3747154870368339796container" dir="ltr"
					  ...
					  <tbody>
								  ...
								  <td style="vertical-align:top;font-size:12px;line-height:16px;font-family:Arial,sans-serif">
									<table id="m_-1220850270213470107m_3747154870368339796summary"
									  ...
									  <tbody>
										<tr>
										  <td style="vertical-align:top;font-size:12px;line-height:16px;font-family:Arial,sans-serif">
											<h3 style="padding-top:20px;font-size:18px;color:rgb(204,102,0);margin:15px 0 0 0;font-weight:normal">
											  Hello ${name},</h3>
											<p style="margin:5px 0 0 0;font:12px/16px Arial,sans-serif">
											  ...
											  your order ships. If you would like to view the status of your order, Track it by clicking <a
												...
												style="text-decoration:none;color:rgb(0,102,153);font:12px/16px Arial,sans-serif"
												target="_blank"
												data-saferedirecturl="${trackingUrl}">here</a> <span class="il"></span></p>
										  </td>
										</tr>
									...
									<table style="width:95%;border-collapse:collapse"
									  ...
									  <tbody>
										...
										<tr>
										  ...
										  <td class="m_-1220850270213470107m_3747154870368339796total"
											style="font-size:14px;font-weight:bold;font:12px/16px Arial,sans-serif;text-align:right;line-height:18px;padding:0 10px 0 0;vertical-align:top;font-family:Arial,sans-serif">
											<strong>$${data.grandTotal}</strong> </td>
										</tr>
										<tr>
						...
						myInvoice.toHtml("./public/images" + filehtml, (err, data) => {
							console.log(data)
						...
						if (req.body.wasReffered) {
							Referral.findByIdAndUpdate(referralid, { $set: { converted: true, orderid: orderidin, amount: ref_total } }, { new: true }).then((data) => console.log({ status: true, data })).catch((err) => console.log({ status: false, error: err }))
						}

						res.status(200).json({ status: true, data })
						if (req.body.userMetaId) {
							UserMeta2.findByIdAndUpdate(req.body.userMetaId, { $push: { orders: data._id } }, { new: true }).then((data) => console.log(data)).catch(console.log)
						}
...
router.post('/add/cart', (req, res) => {
	if (req.body.usermetaid) {
		UserMeta2.findByIdAndUpdate(req.body.usermetaid, { $set: { cart: req.body.cart } }, { new: true }).then((data) => res.json({ status: true, data })).catch((error) => res.json({ status: false, error }))
	} else {
